import { ParameterType } from 'jspsych';
  ...
  class SearchArrayPlugin {
      ...
      trial(display_element, trial) {
		 ...
		 // Assigning number of rows and cols 
		  var nrows = trial.grid_size[0];
		  var ncols = trial.grid_size[1]; 
		  
		  if (nrows*ncols != trial.choices.length) {
			  console.error("Grid dimensions does not match the number of choices");
		  }
		...
		for (var row = 0; row < nrows; row++) {
		  html += '<tr id="jspsych-vsl-grid-scene-table-row-' + row + 
		  '" css="height: ' + trial.image_size[1] + 'px;">';

		  for (var col = 0; col < ncols; col++) {
			  
			var randx = getRndInteger(-trial.image_size[1]*trial.stim_jitter, trial.image_size[1]*trial.stim_jitter)
			var randy = getRndInteger(-trial.image_size[0]*trial.stim_jitter, trial.image_size[0]*trial.stim_jitter)

			html += '<td id="jspsych-vsl-grid-scene-table-' + row + "-" + col +
				'" ' + 'style="padding: ' + trial.image_size[1] / 10 + "px " +
			  trial.image_size[0] / 10 + 'px;">';
			  
			var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
			  html += '<div class="jspsych-search-button" '+
					  'id="jspsych-image-button-response-button-' + i +
					  '" data-choice="' + i + 
					  '", style = "position:relative; left:'+ randx+'px; top:'+randy+'px">' +
					  str + "</div>";
          ...
          function after_response(choice) {
              ...
              var btns = document.querySelectorAll(".jspsych-search-button ");
              for (var i = 0; i < btns.length; i++) {
				  ...
				  btns[i].style.background= cvalue
				//btns[i].style.transform= "scale(2)"
				  }
              }
              // Add audio ////////////////////////
			  var audio = new Audio('media/audio/'+ trial.choices[choice].split(/[/.]/)[2]+ '.wav');
  ...
  SearchArrayPlugin.info = info;

export { SearchArrayPlugin as default };
